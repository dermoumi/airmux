...
#[test]
fn pane_no_command_serializes_to_an_empty_string() {
    let pane = Pane {
        commands: vec![],
        ..Pane::default()
    };

    let output = serde_yaml::to_string(&pane).unwrap();
    let expected_output = "---\n\"\"";
...
#[test]
fn pane_single_command_serializes_to_a_single_string() {
    let pane = Pane {
...
#[test]
fn pane_two_or_more_commands_serializes_to_a_full_object() {
    let pane = Pane {
